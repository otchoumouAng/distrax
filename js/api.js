...
export const api = {
    ...
    /**
     * Enregistre le token FCM de l'appareil auprès du backend.
     * Appeler après connexion réussie, avec la config Firebase du projet.
     *
     * @param {string} firebaseVapidKey  Clé VAPID publique du projet Firebase
     * @param {Object} firebaseConfig    Config du projet Firebase (apiKey, projectId…)
     */
    async registerPushToken(firebaseVapidKey, firebaseConfig) {
        ...
        try {
            // Import dynamique du SDK Firebase
            ...
            if (!token) {
                console.warn('[FCM] Token non obtenu (permissions ou config incorrecte).');
                return;
            }

            // Stocker localement pour pouvoir le supprimer à la déconnexion
            localStorage.setItem('distrax-fcm-token', token);
            ...
            console.warn('[FCM] Erreur d\'initialisation :', err.message);
        }
    },
    ...
    async unregisterPushToken() {
        const token = localStorage.getItem('distrax-fcm-token');
        if (!token) return;
        ...
        try {
            ...
            localStorage.removeItem('distrax-fcm-token');
            console.info('[FCM] Token supprimé ✅');
        } catch (err) {
            console.warn('[FCM] Erreur de suppression du token :', err.message);
        }
    },
